...
/* 			background-color:white; */
   ...
   <?php $tienda = DB::table('tienda')
       ...
       <div class="datocomprobante">
        FECHA: {{ date_format(date_create($notadevolucion->fecharegistro),"d/m/Y H:m:s")  }} <br>
        CÓDIGO: {{ str_pad($notadevolucion->codigo, 8, "0", STR_PAD_LEFT) }} <br>
        SEÑOR(A): {{ $notadevolucion->nombreclienete }} <br>
        DNI/RUC: {{ $notadevolucion->identificacioncliente }}
      </div>
      <table cellspacing="1" >
        <tr>
