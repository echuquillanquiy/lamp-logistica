...
<head>
	<meta charset="UTF-8">
	<title>Devolución de Compra</title>
  <style>
    html,body {
        margin:10px;
        margin-top:0px;
...
<body>
	<div class="cabecera">
      ...
      <?php $tienda = DB::table('tienda')
            ->where('tienda.id',$compradevolucion->idtienda)
            ->first(); ?>
        <p class="text-center">
		...
		<div class="ruc">
          ...
          <h3 style="margin-top:-10px; color:white; padding-top:9px;">DEVOLUCIÓN DE COMPRA</h3>
        </div> 
				<h4 style="margin-top:-10px;">{{ str_pad($compradevolucion->codigo, 8, "0", STR_PAD_LEFT) }}</h4>
			</div>
		</div>
	</div>
	<div class="logo">
	...
	<div class="direcciones">
		<table cellspacing="0" class="table">
			<tr class="table-tr">
          <?php $cli = explode(' - ', $compradevolucion->cliente) ?>
      ...
      <tr class="table-tr">
		      <td class="table-tr-td numero" width="100%" colspan=4>DIRECCIÓN: {{ $compradevolucion->direccionusuariocliente }}, {{ $ubigeocliente->departamento }} - {{ $ubigeocliente->provincia }} -{{ $ubigeocliente->distrito }}  </td>
		  </tr>
      
			<tr class="table-tr">
		      <td class="table-tr-td numero" width="20%" >FECHA: {{ date_format(date_create($compradevolucion->fecharegistro),"d/m/Y H:m:s")  }}</td>
		      <td class="table-tr-td numero" width="20%" >MONEDA: {{ $compradevolucion->monedanombre }}</td>
		      <td class="table-tr-td numero" width="30%" >VENDEDOR: {{ $compradevolucion->nombreusuariovendedor}}</td>
		      <td class="table-tr-td numero" width="30%" >FORMA DE PAGO: {{ $compradevolucion->nombreFormapago}}</td>
		  </tr>
		</table>
		<br>
    <table cellspacing="0" class="table">
