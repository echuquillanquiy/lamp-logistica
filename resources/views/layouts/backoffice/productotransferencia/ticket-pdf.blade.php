...
/* 			background-color:white; */
    ...
    .datocomprobante {
      ...
      <div class="datocomprobante">
          ...
          ?>
        CÓDIGO: {{ str_pad($productotransferencia->codigo, 8, "0", STR_PAD_LEFT) }}<br>
        FECHA: {{ date_format(date_create($productotransferencia->fecharegistro),"d/m/Y H:m:s")  }} <br>
        DE: {{ $productotransferencia->tienda_origen_nombre }}  <br>
        PARA: {{ $productotransferencia->tienda_destino_nombre }}  <br>
        ESTADO: {{ $estado  }}  <br>
		    MOTIVO: {{$productotransferencia->motivo}}
      </div>
      <table cellspacing="1" >
       <tr>
      ...
      <tr class="table-tr">
        <td style="white-space: nowrap;">CANT.</td>
				<td  style="white-space: nowrap;text-align: center;width:60px;">ENV.</td>
				<td  style="white-space: nowrap;text-align: center;width:60px;">REC.</td>
			</tr>
        <tr>
				<td colspan="3" style="text-align: center;height:5px;"><div style="border-top: 1px dashed #31353d;width:100%;"></div></td>
			</tr>
      @foreach($detalletransferencia as $value)
        ...
        <tr>
				<td colspan="3" style="text-align: left;"> {{ $value->productonombre }}</td>
			</tr>
      <tr class="table-tr">
        <td style="white-space: nowrap;text-align: center;">{{ $value->cantidad }}</td>
				...
				<td style="white-space: nowrap;text-align: center;">{{ $value->cantidadrecepcion }}</td>
			</tr>
      @endforeach
        <tr>
				<td colspan="3" style="text-align: center;height:5px;"><div style="border-top: 1px dashed #31353d;width:100%;"></div></td>
