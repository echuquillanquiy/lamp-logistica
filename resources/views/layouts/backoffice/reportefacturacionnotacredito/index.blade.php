...
<div class="panel panel-danger"  data-sortable-id="ui-widget-3">
    <div class="panel-heading ui-sortable-handle">
        <div class="btn-group btn-group-toggle pull-right">
            <a class="btn btn-dark btn-xs" href="{{ url('backoffice/reportefacturacionnotacredito') }}"><i class="fa fa-angle-left"></i> Ir Atras</a>
        </div>
        <h4 class="panel-title">Reporte Notas de Crédito</h4>
    </div>
    <div class="panel-body">
        <form action="{{ url('backoffice/reportefacturacionnotacredito') }}" method="GET" autocomplete="off"> 
          <div class="custom-form">
            <div class="row">
               <div class="col-md-6">
                  ...
                  <label>Correlativo Afectado</label>
                  <input class="form-control" type="text"  name="notacredito_numerodocumentoafectado" id="notacredito_numerodocumentoafectado" value="{{isset($_GET['notacredito_numerodocumentoafectado'])?($_GET['notacredito_numerodocumentoafectado']!=''?$_GET['notacredito_numerodocumentoafectado']:''):''}}">
                  <label>Serie</label>
                  <input class="form-control" type="text"  name="notacredito_serie" id="notacredito_serie" value="{{isset($_GET['notacredito_serie'])?($_GET['notacredito_serie']!=''?$_GET['notacredito_serie']:''):''}}">
                  <label>Correlativo</label>
                  <input class="form-control" type="text"  name="notacredito_correlativo" id="notacredito_correlativo" value="{{isset($_GET['notacredito_correlativo'])?($_GET['notacredito_correlativo']!=''?$_GET['notacredito_correlativo']:''):''}}">                
               </div>
               <div class="col-md-6">
        ...
        <div class="table-responsive">
             @include('layouts.backoffice.reportefacturacionnotacredito.tabla')
...
function reporte(tipo){
    window.location.href = '{{url('backoffice/reportefacturacionnotacredito')}}?'+
      'tipo='+tipo+
      '&tipoCompbrobante='+($('#tipoCompbrobante').val()!=null?$('#tipoCompbrobante').val():'')+
      '&notacredito_numerodocumentoafectado='+$('#notacredito_numerodocumentoafectado').val()+
      '&notacredito_serie='+$('#notacredito_serie').val()+
      '&notacredito_correlativo='+$('#notacredito_correlativo').val()+
