...
/* 			background-color:white; */
   ...
   <?php $tienda = DB::table('tienda')
      ...
      <div class="datocomprobante">
        CÓDIGO: {{ str_pad($venta->codigo, 8, "0", STR_PAD_LEFT) }}<br>
        FECHA: {{ date_format(date_create($venta->fecharegistro),"d/m/Y H:m:s")  }} <br>
        SREÑOR(A):  {{$venta->nombreclienete }}<br>
        DNI/RUC: {{$venta->identificacioncliente }}
      </div>
