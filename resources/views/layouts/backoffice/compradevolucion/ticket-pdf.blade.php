...
/* 			background-color:white; */
   ...
   <?php $tienda = DB::table('tienda')
        ...
        <p style=" text-align:center;">{{strtoupper($tienda->nombre)}}</p>
        <p >{{$tienda->correo}}</p>
        <p>{{$tienda->numerotelefono}}</p>
 
       <div class="datocomprobante">
        FECHA: {{ date_format(date_create($compradevolucion->fecharegistro),"d/m/Y H:m:s")  }} <br>
        CÓDIGO: {{ str_pad($compradevolucion->codigo, 8, "0", STR_PAD_LEFT) }} <br>
        SEÑOR(A): {{$compradevolucion->nombrecliente }} <br>
        DNI/RUC: {{$compradevolucion->identificacioncliente }}
      </div>
      <table cellspacing="1" >
        <tr>
